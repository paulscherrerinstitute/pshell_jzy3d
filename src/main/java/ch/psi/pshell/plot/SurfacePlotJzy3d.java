...
public class SurfacePlotJzy3d extends SurfacePlotBase {
    ...
    static void extractLibraries() throws IOException {
        ...
        String jar = IO.getExecutingJar(SurfacePlotJzy3d.class);
        if (jar != null) {
            ...
            for (String lib : libs) {
                try {
                    ...
                    if (!libFile.exists()) {
                        Logger.getLogger(SurfacePlotJzy3d.class.getName()).log(Level.INFO, "Extracting: " + resourceName);
                        IO.extractZipFileContent(new File(jar), resourceName, libFile.getCanonicalPath());

                    }
                } catch (Throwable ex) {
                    Logger.getLogger(SurfacePlotJzy3d.class.getName()).log(Level.SEVERE, "Error extracting: " + lib + " - " + ex.getMessage());
